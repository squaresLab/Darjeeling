...
class PlusCollectorConfig(CoverageCollectorConfig):
    NAME: t.ClassVar[str] = 'plus'
    ...
    def from_dict(
        ...
        config = PlusCollectorConfig(files_to_instrument=files_to_instrument)
        logger.trace(f"plus config: {config}")
        return config
...
@attr.s(frozen=True, slots=True, auto_attribs=True)
class PlusCollector(CoverageCollector):
    program: 'ProgramDescription'
