...
class GenProgStrategy(Strategy):
    ...
    def evaluate(self, pop: Population) -> None:
        # submit all to the evaluator
        for ind in pop:
            # TODO choose tests
            future = evaluator.submit(ind, tests)
    ...
    def run(self) -> Iterator[Candidate]:
        ...
        for g in range(0, self.num_generations + 1):
            ...
            # choose tests
            self.evaluate(pop)  # TODO test sampling
            pop = self.select(pop)

            # future_outcome = evaluator.submit(candidate, tests)
            # outcome = future_outcome.result()
